...
class SnapsProducer(dai.node.HostNode):
    ...
    def build(self, rgb: dai.Node.Output, detections: dai.Node.Output, label_map: list, confidence_threshold: float = 0.7, labels: list = ["person"], time_interval: float = 60.0) -> None:
    ...
    def process(self, rgb: dai.Buffer, detections: dai.ImgDetections):
        for det in detections.detections:
            if (det.confidence >= self.confidence_threshold and self.label_map[det.label] in self.labels and time.time() > self.last_update + self.time_interval):
                ...
                self.em.sendSnap("rgb", rgb, [], ["demo"], extra_data)
